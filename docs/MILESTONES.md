...
| **Watch memory as M2 adds the verse index.** Headroom is about **100 MB**, not the 260 MB the faulty 40 MB reading implied: a dev build idles near 200 MB against a 300 MB budget. M2 loads the 7.71 MB int8 verse index and the 7.97 MB encoder matrix into memory, and adds a scripture cache; none of that is large on its own, and together with a 45-minute transcript and the rolling buffer it is worth measuring rather than assuming. `scripts/measure-memory.ps1` reports the private working set across the whole process tree. Measure before and after the vector stage lands, not at the end of M2. | M2 (throughout) | 23 Sept 2026 |
