...
| **Vendor API keys: decided, not yet built.** The operator has chosen a **hybrid** model and written it up in `docs/SermonAI_API_Key_Strategy_Prompt.md`: a SermonAI Gateway holds the real keys and each installation gets its own install token, with optional bring-your-own-key for churches that want it, stored in the OS keychain. That supersedes the open question — the app never sees our keys in managed mode, which is the only arrangement where "not in the installer" also means "not on their machine". PRD §17 still describes the older idea (keys provisioned into the church's keychain) and needs amending to match, along with its neighbouring "no account required" line, since an install token is an identity. §17 is annotated pending that. Note the strategy doc also names a fourth service not in the PRD: the **Tyndale NLT API**. Sequencing is the operator's: the doc is to be picked up **after M1 deliverable 4**, at the latest before M4, and runs in phases with STOP points — including one before any Cloudflare account is created, which is theirs to do, not ours. Nothing ships keys today: `.env` is dev-only and compiled out of release builds, verified against the release binary. | M1 (after deliverable 4) → M4 | 22 Sept 2026 |
