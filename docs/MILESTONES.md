...
**Definition of Done**
- [ ] Play a 10-minute sermon recording through a virtual audio device on each platform: transcript appears with under 700 ms lag (p99), no dropped words visible on inspection. *(Instrumented and ready to run: `run()` records lag per settled utterance, the footer shows `lag p99` live, and stopping logs p50/p95/p99/max against the 700 ms budget. `scripts/make-test-sermon.ps1` builds the audio and `docs/testing/m1-dod.md` has the steps. **Windows needs no virtual audio device** — WASAPI loopback makes playing a file through the speakers a routed signal path, which also exercises the code a church uses for a desk feed. macOS does need BlackHole.)*
...
- [ ] RAM during capture under 300 MB. *(**152 MB idle** on a dev build, measured as private working set across the whole process tree. Not yet measured under capture, and the number of record needs a release build — a `tauri dev` run carries an unoptimized binary and a live Vite server. `scripts/measure-memory.ps1` reports it; run it alongside the 10-minute test and both lines close together.*
  - *Summing `WorkingSet` across the tree gave **583 MB**, which would have failed the budget: a Tauri app is nine processes here and `WorkingSet` counts pages shared between them once per process.)*
