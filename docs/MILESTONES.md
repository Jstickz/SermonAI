...
| **YouVersion search endpoints** are named in PRD §15.2 but not implemented. User-typed verse lookup currently resolves through `bible::reference::parse` plus a passage fetch, which covers a known reference but not a free-text search ("the one about the mustard seed"). The command palette is where this surfaces. | M3 (command palette, FR-31) | 21 Sept 2026 |
| **Untitled Serif font rule** (PRD §15.2) is not implemented. YouVersion serve their typeface through a Fonts API and it must be loaded live rather than self-hosted. The projector is unaffected — it uses Crimson Pro, which §15.2 explicitly permits — so this only bites where YouVersion's own formatting stands, which is the summary PDF. | M4 (summary PDF) | 21 Sept 2026 |
| **Display-side attribution refusal.** §15.2 requires the app to refuse to display a verse whose version returns no attribution, and show a fallback message instead. The data layer enforces this today (`has_attribution`, and the build script will not write a pack without a copyright string), but the refusal itself belongs to the renderers, which do not exist yet: the projector must show the fallback rather than bare text, and the PDF must skip the block and log a warning. | M3 (projector) and M4 (PDF) | 21 Sept 2026 |
