...
**Current milestone:** M1 — Audio In, Transcript Out (reopened)
...
**Definition of Done**
- [ ] **FAILING.** Play a 10-minute sermon recording through a virtual audio device on each platform: transcript appears with under 700 ms lag (p99), no dropped words visible on inspection. *(23 Sept: a release-build run reported **2,797 ms p99**, four times the budget. Investigated before M2 — see `docs/testing/m1-latency-baseline.md`. **Both measurement faults are fixed and the line needs re-running before it can be ticked.**
"
  "  - **It measured finals only.** Lag was recorded in `push_final` and nowhere else, so it described when Deepgram *confirms* an utterance — which it does only after deciding the speaker has stopped. The line says "transcript **appears**", and words appear as interim results. Interim and settled are now measured separately.
"
  "  - **It charged the WebSocket handshake to every sample.** The clock started at capture; Deepgram times its words from the first byte *it* receives. The handshake measured **1,188–1,365 ms** against the live service, and all of it was being counted as lag. The clock is now anchored to the first chunk sent.
"
  "  - **Measured after both fixes**, on 38 s of speech against the live service: words appear at **~452 / ~681 / ~698 ms** (p50/p95/p99), of which 250 ms is our chunking and the rest is network plus Deepgram. That is inside the 700 ms line, marginally, and inside PRD §18.1's 800 ms end-to-end regex path. Settled text is **~811 / ~2,289 ms**, which is Deepgram's endpointing.
"
  "  - **Endpointing is not tunable enough to matter**: default, 100 ms and 300 ms produced 2,039 / 2,083 / 1,664 ms p95, within variance on ten samples. Roughly 2 s should be treated as the cost of a confirmed utterance.
"
  "  - Those figures come from synthesised speech on one machine and network, n=27. **A real run is what closes this line**, and the app now records both numbers itself — the footer shows p50/p95/p99 for each, and stopping logs them.)*
...
- [ ] **UNVERIFIED.** RAM during capture under 300 MB. *(23 Sept: a release run reported **40 MB**, which was a fourfold undercount. `measure-memory.ps1` resolved processes through performance-counter *instance names*, and several processes are all called `msedgewebview2`, so the lookup collided. Measured against a live app it resolved **1 of 9 processes** and reported 6.3 MB where the true figure was **193.4 MB**. The 40 MB was `sermonai.exe` alone, which looked plausible because the Rust process genuinely is small — 6.3 MB private — while WebView2 carries almost all of the app's memory.
"
  "  - **Fixed**: processes are matched by PID through `Win32_PerfRawData_PerfProc_Process`, which cannot be confused by duplicate names, and every sample prints how many it resolved. A run resolving one process now declares itself invalid.
"
  "  - **Re-measure needed.** A dev build idles at ~200 MB against a 300 MB budget — far less headroom than 40 MB implied, and the release figure under capture is still unknown.)*

**M1 deliverables: 10 of 10. Definition of Done: 2 of 4 on Windows — the lag line is failing and the memory line is unverified, both after their first figures turned out to be measurement faults. M1 is reopened.**
...
| **PRD §18.1's end-to-end regex path may be unreachable as written.** It budgets 800 ms p99 from spoken word to output. Measured, words *appear* at ~698 ms p99 but are not *confirmed* until ~2,289 ms, and Deepgram's endpointing is not tunable enough to change that — default, 100 ms and 300 ms all landed within variance. Detection runs on confirmed text (the interim/final split exists precisely to stop it acting on words that may be revised), so a spoken reference cannot reach the projector inside 800 ms however fast the regex stage is. Either §18.1 measures from settled text onward and should say so, or the budget moves. A decision before M2's detection pipeline, not after. | M2 (detection pipeline) | 23 Sept 2026 |
