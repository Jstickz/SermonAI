...
- Endpoints the app uses: `GET /bibles` (list versions the app key can access), `GET /bibles/{version_id}` (metadata plus attribution text), `GET /bibles/{version_id}/passages/{passage_id}` (the passage text), and the search endpoints for user-typed lookups.

- **`?format=html` is required, and its absence is silent.** The passages endpoint returns **plain text by default** — a whole chapter arrives as one unbroken string with no verse markers, no paragraph structure and nothing to split on. Passing `?format=html` is what returns markup with verse boundaries. Nothing errors without it; you simply get text that cannot be divided into verses, which is why both the Rust client and `build-translation-pack.py` always append it.

- **Verse markup shape.** With `format=html`, a verse is marked by `<span class="yv-v" v="16"></span>` and its printed number appears separately in `<span class="yv-vlbl">16</span>`. Verse identity lives in a bare `v` attribute, not in the class and not in a `data-` attribute; the label span is typography and is dropped. Poetry is wrapped in `q1`/`q2` divs, so a single verse can span several of them.
...
- **Three behaviours confirmed against the live API on 21 September 2026**, none of them obvious from the endpoint list:
  1. `copyright` is **null on `GET /bibles`** and populated only on `GET /bibles/{version_id}`. Attribution therefore requires a per-version fetch; the list must never be treated as a source of it.
  2. `GET /bibles` **requires `language_ranges[]`** (e.g. `?language_ranges[]=eng`) and rejects the request without it. It is also paginated, returning `data`, `next_page_token` and `total_size`. Note that `curl` treats `[]` as glob syntax and needs `-g`, or the parameter never arrives.
  3. **No licence field exists on any response.** `GET /bibles` returns exactly the versions this app key may use, so licence state is derived from presence in that list rather than read from a field.

- Response envelopes are inconsistent: `GET /bibles` wraps its payload in `data`, while `GET /bibles/{version_id}` and the passages endpoint return bare objects. Parsers must handle both, and must not "try bare, then try enveloped" — with optional fields an envelope parses cleanly as an empty object and yields a blank verse instead of an error.
