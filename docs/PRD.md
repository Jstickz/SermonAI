...
- **FR-59:** The base installer for each platform (`.msi`/`.exe` for Windows, `.dmg` for macOS — unsigned during development, signed and notarized before M11 public launch, see §9.5) shall be **under 40 MB** and shall include: the application, **the bundled translations listed below**, the quantized verse embedding index, and the default theme. It shall install without administrator rights and without a setup wizard beyond audio device and display selection.
  - **Bundled translations (amended 21 September 2026):** **WEB** only, pending a decision on what joins it. The original list of KJV, WEB and ASV is no longer buildable. **KJV is not licensed to our YouVersion app key** — the only King James in their 1,485-version catalog is Thai — and **ASV returns no copyright string**, which §15.2 forbids displaying. A KJV pack built from ebible.org is staged in `staging/translations/` but is not enabled; see that directory's README for the UK Crown copyright question. Remaining public-domain candidates are BSB (3034), Geneva (2163) and FBV (1932, CC BY-SA).
