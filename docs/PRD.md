...
- **FR-59:** The base installer for each platform (`.msi`/`.exe` for Windows, `.dmg` for macOS — unsigned during development, signed and notarized before M11 public launch, see §9.5) shall be **under 40 MB** and shall include: the application, **the bundled translations listed below**, the quantized verse embedding index, and the default theme. It shall install without administrator rights and without a setup wizard beyond audio device and display selection.
  - **Bundled translations:** **KJV, WEB and ASV**, as originally specified. All three are public domain.
    - They do not all come from the same place, and that is deliberate. WEB is built from YouVersion, which licenses it to our app key and returns the attribution `PUBLIC DOMAIN (not copyrighted)`. **KJV and ASV are built from ebible.org** instead: YouVersion does not license a King James to our key (the only one in their 1,485-version catalog is Thai) and returns no copyright string for their ASV. Neither limitation is a property of the text — both are public domain and freely redistributable — so they are taken from a source whose terms permit it. YouVersion's terms govern YouVersion's copy only.
    - Every bundled pack still carries an attribution line, even though public-domain text obliges none. §14.2 makes `attribution` NOT NULL and §15.2's renderers refuse a verse without one, so a pack with an empty string would be rejected by our own guard. Each states its public-domain status and its source, e.g. “American Standard Version (1901). Public domain. Text from eBible.org.”
    - The ASV omits sixteen verses that the KJV prints, among them Matthew 17:21, Mark 9:44 and Acts 8:37; the 1901 revisers followed a Greek text lacking those readings and moved them to the margin. The pack records them in an `omitted_verses` list so a lookup can say the translation omits the verse rather than project a blank.
    - The UK Crown copyright question on the KJV is unresolved and recorded in the pack manifest: public domain in the US, but under perpetual Crown copyright in the UK, administered by Cambridge University Press under letters patent. §4.2 lists the UK as a target market, so this needs a deliberate decision before UK distribution.
  - Only translations we may **redistribute** belong in the installer. Licensed translations such as NIV, AMP and NASB are fetched through the API under §15.2 and cached locally; they are not packaged into the download.
